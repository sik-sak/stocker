...
logging.basicConfig(
    filename=os.path.join("Server/Logs", "historicalInsertLogs.log"),
    level=logging.INFO,
...
# Prepare to insert data into Snowflake
query = f"""
    INSERT INTO {snowflake_table}(symbol, company_name, current_price, market_capital, pe_ratio, dividend_yield, historical_data, updated_at) VALUES (%s, %s, %s, %s, %s, %s, %s, '{updated_at}')
"""
...
def upload(symbol, companyName, currentPrice, marketCap, pe_ratio, dividend_yield, historical_data):
    """
    Inserts stock data in the Snowflake database.

    This function attempts to insert stock data into the Snowflake database. It includes details such as the symbol, company name, current price, market capitalization, P/E ratio, dividend yield, and historical data.
    ...
    Raises:
    Exception: If the database insert operation fails, the exception is caught and an error message is printed.
    """
    try:
